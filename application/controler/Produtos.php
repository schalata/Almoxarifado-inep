...
class Produtos {
	...
	function Listar(){
		...
		$produtos=$this->conexaoDB->PesquisaCampos("id,nome,ficha",$this->tabela);
		
		echo '<h1>Listando '.$this->tabela.'</h1><br><br>';
		echo '<table align="center">';
		//echo '<tr><td class="header">ID</td><td class="header">Nome</td><td></td><td></td></tr>';
		echo '<tr><td class="header">ID</td><td class="header">Nome</td><td class="header">Ficha técnica</td><td></td></tr>';
		...
		while($produto=mysql_fetch_array($produtos)){
			...
			echo '<td>'.$produto['nome'].'</td>';
                        //echo '<td><a href="'.$produto['ficha'].'">'.$produto['nome'].'</a></td>';
                        echo '<td><a href="'.$produto['ficha'].'">Clique aqui</a></td>';
	...
	function Saida($produto,$quantidade){
		...
		echo 'A quantidade a Ser Retirada é maior do que a existente';
		exit();
		}
		else
		{
